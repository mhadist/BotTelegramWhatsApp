...
spreadsheet_key = '1N_01UHFpqrcLOieBB6W_AhZbiT8WphBZbW338WMyDcs'
credentials = ServiceAccountCredentials.from_json_keyfile_name('botmagang-405867e41e12.json', ['https://www.googleapis.com/auth/spreadsheets'])
...
def show_data(context):
    ...
    context.bot.send_message(chat_id='-1001426872464', text=f'```\n{table}\n```', parse_mode='MarkdownV2')
...
def show_data_report(context):
    ...
    table = tabulate(rows, headers)
    context.bot.send_message(chat_id='-626691214', text=f'```\n{table}\n```', parse_mode='MarkdownV2')
...
def record_data(context):
    ...
    context.bot.send_message(chat_id='-1001426872464', text='Data telah direkam.')

def main():
    # Inisialisasi bot
    updater = Updater(token='6041030781:AAEkqOrAU2jIhMmg78lcjYFDbGoJ6p-ioVw', use_context=True)
    ...
    schedule.every().hour.at(":15").do(show_data, context=updater)
    schedule.every().hour.at(":15").do(show_data2, context=updater)
    schedule.every().hour.at(":15").do(show_data3, context=updater)
    schedule.every().hour.at(":15").do(record_data, context=updater)
    schedule.every().hour.at(":15").do(show_data_report, context=updater)
